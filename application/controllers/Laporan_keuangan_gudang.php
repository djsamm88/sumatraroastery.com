...
class Laporan_keuangan_gudang extends CI_Controller {
	public function __construct()
	{
		...
		$this->load->model('m_laporan_keuangan_gudang');
	...
	public function kas()
	{
		$data['kas'] = $this->m_laporan_keuangan_gudang->m_sisa_kas();
		$data['laporan_group'] = $this->m_laporan_keuangan_gudang->m_laporan_group();
		$this->load->view('gudang/table_sisa_kas',$data);
	}
	...
	public function arus_kas()
	{
		...
		$data['laporan_group'] = $this->m_laporan_keuangan_gudang->m_laporan_group();
		$this->load->view('gudang/tbl_arus_kas',$data);
	}

	public function arus_kas_paket()
	{
		
		$data['laporan_paket'] = $this->m_laporan_keuangan_gudang->m_saldo_per_paket();
		$this->load->view('gudang/tbl_arus_kas_paket',$data);
	}
	...
	{
		$this->load->view('gudang/form_penambahan_saldo');
	}

	public function form_penarikan_saldo()
	{
		$this->load->view('gudang/form_penarikan_saldo');
	}

	public function form_koreksi()
	{
		$this->load->view('gudang/form_koreksi');
	}
	...
	public function by_tgl($tgl)
	{
		...
		$data['all'] = $this->m_laporan_keuangan_gudang->m_by_tgl($tgl);
	...
	public function go_koreksi()
	{
		...
		$q = $this->m_laporan_keuangan_gudang->m_by_id($id);
	...
	public function laporan_jurnal()
	{
		...
		$data['all'] = $this->m_laporan_keuangan_gudang->m_jurnal($tgl_awal,$tgl_akhir);
		$data['tgl_awal'] = $tgl_awal;
		$data['tgl_akhir'] = $tgl_akhir;
		$this->load->view('gudang/table_jurnal',$data);
	}


	public function laporan_jurnal_pelanggan($id_pelanggan)
	{
		$data['all'] = $this->m_laporan_keuangan_gudang->m_jurnal_pelanggan($id_pelanggan);		
		$data['pelanggan'] = $this->m_pelanggan->m_by_id($id_pelanggan)[0];		
		$this->load->view('gudang/table_jurnal_pelanggan',$data);
	}
	...
	public function laporan_laba()
	{
		$tgl_awal = $this->input->get('tgl_awal');
		$tgl_akhir = $this->input->get('tgl_akhir');
		$data['all'] = $this->m_laporan_keuangan_gudang->m_laba($tgl_awal,$tgl_akhir);
		$data['tgl_awal'] = $tgl_awal;
		$data['tgl_akhir'] = $tgl_akhir;
		$this->load->view('gudang/table_laba',$data);
	...
	public function laporan_laba_pdf()
	{
		...
		if (file_exists($pdfFilePath) == FALSE)
		{
			...
			$html = $this->load->view('gudang/table_laba_pdf.php',$data,true);
	...
	public function laporan_jurnal_pdf()
	{
		...
		if (file_exists($pdfFilePath) == FALSE)
		{
        	...
        	ini_set('memory_limit', '2048M');
			//$html = $this->load->view('laporan_mpdf/pdf_report', $data, true); // render the view into HTML
			$html = $this->load->view('gudang/table_jurnal_pdf.php',$data,true);
	...
	public function detail_arus_kas()
	{
		...
		$data['all'] = $this->m_laporan_keuangan_gudang->m_detail_arus_kas($id_group,$tgl_awal,$tgl_akhir);
		$data['tgl_awal'] = $tgl_awal;
		$data['tgl_akhir'] = $tgl_akhir;
		$this->load->view('gudang/detail_arus_kas',$data);
	}
	...
	public function detail_arus_kas_paket()
	{
		...
		$data['all'] = $this->m_laporan_keuangan_gudang->m_detail_arus_kas_paket($id_paket,$tgl_awal,$tgl_akhir);
		$data['tgl_awal'] = $tgl_awal;
		$data['tgl_akhir'] = $tgl_akhir;
		$this->load->view('gudang/detail_arus_kas',$data);
	}
	...
	public function detail_arus_kas_paket_pdf()
	{
		...
		if (file_exists($pdfFilePath) == FALSE)
		{
        	...
        	ini_set('memory_limit', '2048M');
			//$html = $this->load->view('laporan_mpdf/pdf_report', $data, true); // render the view into HTML
			$html = $this->load->view('gudang/detail_arus_kas_paket_pdf.php',$data,true);
